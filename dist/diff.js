 ...
 */
import { canonicalJson, loadRun } from "./runfile.js";
...
function signature(e) {
    if (e.kind === "llm") {
        if (e.fingerprint)
            return "llm:" + e.fingerprint;
        const req = (e.request ?? {});
        return "llm:" + canonicalJson({ model: req["model"], messages: req["messages"] });
    }
    if (e.kind === "tool")
        return "tool:" + String(e.name);
    if (e.kind === "span")
        return `span:${e.phase}:${e.name}`;
...
export function diffRuns(a, b) {
    ...
    for (let i = 0; i < n; i++) {
        const ea = (ra.events[i] ?? null);
        const eb = (rb.events[i] ?? null);
        let same = null;
        if (ea && eb) {
            same = signature(ea) === signature(eb);
        }
        if (same === false && firstDivergence === null)
            firstDivergence = i;
        if (same !== true)
            identical = false;
        steps.push({
